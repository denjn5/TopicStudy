...
Chris Ward, 2014
https://github.com/cmward/hobbs
tree: http://stackoverflow.com/questions/36610179/how-to-get-the-dependency-tree-with-spacy
"""
...
def get_pos(tree, node):
    """ Given a tree and a node, return the tree position of the node.
    """
...
def get_dom_np(sents, pos):
    """ Finds the position of the NP that immediately dominates the pronoun.
    :param sents: List of trees (or tree) to search
    :param pos: The tree position of the pronoun to be resolved
    :return:
...
def walk_to_np_or_s(tree, pos):
    """ Takes the tree being searched and the position from which the walk up is started. Returns the
    position of the first NP or S encountered and the path taken to get there from the
    dominating NP. The path consists of a list of tree positions.
    :param tree: The tree being searched
    :param pos: The position from which the walk is started
    :return:
...
def traverse_left(tree, pos, path, pro, check=1):
    ...
    # convert the tree positions of the subtree rooted at pos
    ...
    if check == 1:
        for p in bf_pos:
            if p < path[0] and p not in path:
                if "NP" in tree[p].label() and match(tree, p, pro) and check_for_intervening_np(tree, pos, p, pro):
                    return tree, p
