...
# RUN
# Get the Corpus
new_texts: dict = get_bible.main(NEW_CORPUS)  # Get properly formatted corpus (dictionary: { reference: text })
if USE_GRAPH_DB:
    get_bible.db_add_posts(new_texts, db_start_fresh=False)

# Run it through Topic Builder (tokenizer, graph db set up, find topics)
tb: ana_topics.TopicBuilder = ana_topics.TopicBuilder(NEW_CORPUS, new_texts, USE_GRAPH_DB)
...
if ORIG_CORPUS:
    # Get the Corpus
    orig_texts: dict = get_bible.main(ORIG_CORPUS)
    get_bible.db_add_posts(orig_texts, db_start_fresh=True)

    # Run it through Topic Builder (tokenizer, graph db set up, find topics)
    orig_tb: ana_topics.TopicBuilder = ana_topics.TopicBuilder(ORIG_CORPUS, orig_texts, use_graph_db=False)
